...
#ifndef _XFER_XNOOP_H_
#define _XFER_XNOOP_H_

#include "xferPlugin.h"
#include "gtcd.h"
#include "xferPlugin_gtc_prot.h"

class xferPlugin_xnoop : public xferPlugin {
...
private:
    ...
    void get_chunk(ref<dot_descriptor> d, ref<vec<oid_hint> > hints,
                   chunk_cb cb, CLOSURE);
    void get_descriptors_cb(descriptors_cb cb1, str s,
...
public:
    ...
    xferPlugin_xnoop(gtcd *m, xferPlugin *next_xp) : m(m), xp(next_xp)
        { assert(m); assert(xp); }
    ~xferPlugin_xnoop() { }
};

#endif /* _XFER_XNOOP_H_ */
